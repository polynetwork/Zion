...
func TestTraceCall(t *testing.T) {
	...
	params.GenesisSupply = big.NewInt(params.Ether * 3)
	genBlocks := 10
	...
	api := NewAPI(newTestBackend(t, genBlocks, genesis, func(i int, b *core.BlockGen) {
		...
		//    value: 1000 wei
		//    fee:   0 wei
		tx, _ := types.SignTx(types.NewTransaction(uint64(i), accounts[1].addr, big.NewInt(1000), params.TxGas, big.NewInt(params.InitialBaseFee), nil), signer, accounts[0].key)
		b.AddTx(tx)
	}))
