...
//	}
//
//	log.Trace("ZionMainChainHandler", "MakeDepositProposal", params.String())
//
//	sideChain, err := side_chain_manager.GetSideChain(s, params.SourceChainID)
//	if err != nil {
//		return nil, fmt.Errorf("ZionMainChainHandler MakeDepositProposal, side_chain_manager.GetSideChain err: %v", err)
//	}
//	if sideChain == nil {
//		return nil, fmt.Errorf("ZionMainChainHandler MakeDepositProposal, side chain not found")
//	}
//
//	val := &common.MakeTxParam{}
//	if err := val.Deserialization(pcom.NewZeroCopySource(params.Extra)); err != nil {
//		return nil, fmt.Errorf("ZionMainChainHandler MakeDepositProposal, failed to deserialize MakeTxParam: %v", err)
//	}
//	if err := common.CheckDoneTx(s, val.CrossChainID, params.SourceChainID); err != nil {
//		return nil, fmt.Errorf("ZionMainChainHandler MakeDepositProposal, check done transaction error: %v", err)
//	}
//
//	header := &types.Header{}
//	if err := json.Unmarshal(params.HeaderOrCrossChainMsg, header); err != nil {
//		return nil, fmt.Errorf("ZionMainChainHandler MakeDepositProposal, deserialize header err: %v", err)
//	}
//
//	curEpochStartHeight, curEpochValidators, err := zion.GetEpoch(s, params.SourceChainID)
//	if err != nil {
//		return nil, fmt.Errorf("ZionMainChainHandler MakeDepositProposal, failed to get current validators height: %v", err)
//	}
//	if header.Number.Uint64() < curEpochStartHeight {
//		return nil, fmt.Errorf("ZionMainChainHandler MakeDepositProposal, height of header %d is less than epoch height %d", header.Number.Uint64(), curEpochStartHeight)
//	}
//
//	if _, _, err := zion.VerifyHeader(header, curEpochValidators, false); err != nil {
//		return nil, fmt.Errorf("ZionMainChainHandler MakeDepositProposal, failed to verify quorum header %s: %v", header.Hash().String(), err)
//	}
//
//	if err := xutils.VerifyTx(params.Proof, params.Extra, header, ecom.BytesToAddress(sideChain.CCMCAddress)); err != nil {
//		return nil, fmt.Errorf("ZionMainChainHandler MakeDepositProposal, verifyFromEthTx err: %v", err)
//	}
//
//	if err := common.PutDoneTx(s, val.CrossChainID, params.SourceChainID); err != nil {
//		return nil, fmt.Errorf("ZionMainChainHandler MakeDepositProposal, PutDoneTx error: %v", err)
//	}
//
//	return val, nil
//}
