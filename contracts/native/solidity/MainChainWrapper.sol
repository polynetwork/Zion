...
contract MainChainWrapperTest {
    ...
    constructor() public {
        lockProxy = address("0x7d79D936DA7833c7fe056eB450064f34A327DcA8");
        asset = address(0);
    }
    ...
    function lock(uint64 toChainId, address toAddress, uint256 amount, uint256 fee) external payable returns (bool) {
        ...
        bytes memory toAddr = abi.encodePacked(toAddress);
        require(IMainChainLockProxy(lockProxy).lock(asset, toChainId, toAddr, amount), "lock failed");
