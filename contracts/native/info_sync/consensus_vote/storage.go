...
func voteMessageKey(hash common.Hash) []byte {
	return utils.ConcatKey(utils.InfoSyncContractAddress, []byte(SKP_VOTE_MESSAGE), hash.Bytes())
}

func signerMapKey(hash common.Hash) []byte {
	return utils.ConcatKey(utils.InfoSyncContractAddress, []byte(SKP_SIGNER_MAP), hash.Bytes())
}
