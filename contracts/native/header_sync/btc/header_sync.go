...
func (this *BTCHandler) SyncGenesisHeader(native *native.NativeContract) error {
	...
	header, height, err := getGenesisHeader(params)
	if err != nil {
		return fmt.Errorf("BTCHandler SyncGenesisHeader: %s", err)
	}

	headerStore, err := native.GetCacheDB().Get(utils.ConcatKey(utils.HeaderSyncContractAddress, []byte(hscommon.GENESIS_HEADER), utils.GetUint64Bytes(params.ChainID)))
	if err != nil {
...
func (this *BTCHandler) SyncBlockHeader(native *native.NativeContract) error {
	ctx := native.ContractRef().CurrentContext()
	headerParams := &hscommon.SyncBlockHeaderParam{}
	if err := utils.UnpackMethod(hscommon.ABI, hscommon.MethodSyncBlockHeader, headerParams, ctx.Payload); err != nil {
...
func (this *BTCHandler) SyncCrossChainMsg(native *native.NativeContract) error {
	return nil
}

func getGenesisHeader(params *hscommon.SyncGenesisHeaderParam) (*wire.BlockHeader, uint32, error) {
...
// error info
func commitHeader(native *native.NativeContract, chainID uint64, header wire.BlockHeader) (bool, *StoredHeader, uint32, error) {
	newTip := false
