...
func (s *SignerImpl) VerifyHeader(header *types.Header, valSet hotstuff.ValidatorSet, seal bool) (*types.HotstuffExtra, error) {
	...
	if seal {
		...
		if err := s.checkQuorum(valSet, header.Hash(), extra.CommittedSeal, true); err != nil {
			return extra, err
		}
	}
...
func (s *SignerImpl) VerifyQC(qc hotstuff.QC, valSet hotstuff.ValidatorSet) error {
	...
	return s.checkQuorum(valSet, hash, committedSeal, false)
}
...
func (s *SignerImpl) checkQuorum(valset hotstuff.ValidatorSet, hash common.Hash, seals [][]byte, salt bool) error {
	var (
