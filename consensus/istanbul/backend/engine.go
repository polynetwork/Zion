...
func (sb *backend) snapshot(chain consensus.ChainReader, number uint64, hash common.Hash, parents []*types.Header) (*Snapshot, error) {
	...
	for snap == nil {
		...
		if number == 0 {
			var err1 error
			snap, err1 = sb.getGenesisSnap(chain)
			if err1 != nil {
				return nil, err1
			}
			sb.genesisSnap = snap
...
func (sb *backend) candidatesLoop() {
	go func() {
		for {
			select {
			case view := <-sb.candidateCh:
				sb.addCandidate(view.chain, view.number)
			}
		}
	}()
}

func (sb *backend) addCandidate(chain consensus.ChainReader, number uint64) {
	var (
		governanceAddress = common.HexToAddress(native.GovernanceContractAddress)
		emptyHash         = common.Hash{}
		startBlkNum       = number - 3
		// todo(fuk): modify period to 1024
		period uint64 = 20
	)

	if startBlkNum < 0 || startBlkNum%period != 0 {
		return
	}

	// get stateDB with header and chain reader
	header := chain.GetHeaderByNumber(startBlkNum)
	if header == nil || header.Root == emptyHash {
		log.Error("[istanbul consensus]", "add candidates header error", "invalid header")
		return
	}
	c, ok := chain.(*core.BlockChain)
	if !ok {
		log.Error("[istanbul consensus]", "add candidates chain error", "invalid blockchain")
		return
	}
	stateDB, _, err := c.StateAt(header.Root)
	if err != nil {
		log.Error("[istanbul consensus]", "add candidates root error", err)
		return
	}
	...
	if err != nil {
		log.Error("[istanbul consensus]", "add candidates snap err", err.Error())
		return
	}
	if sb.genesisSnap == nil {
		genesisSnap, err := sb.getGenesisSnap(chain)
		if err != nil {
			log.Error("[istanbul consenesus]", "load genesis snap error", err)
			return
		}
		sb.genesisSnap = genesisSnap
	}

	// assemble nodes' address list
	lastValSet := validator.NewSet(snap.validators(), sb.config.ProposerPolicy)
	for _, v := range sb.genesisSnap.validators() {
		lastValSet.RemoveValidator(v)
	}

	vst := governance.NewValidatorStorage(stateDB, governanceAddress)
	curValSet := validator.NewSet(vst.GetValid(), sb.config.ProposerPolicy)
	for _, v := range sb.genesisSnap.validators() {
		curValSet.RemoveValidator(v)
	}

	addList, delList := validator.Subtraction(lastValSet, curValSet)
	...
	for _, v := range addList {
		sb.candidates[v] = true
		log.Debug("[istanbul consensus]", "add candidates", v.String(), "block number", number)
	}
	for _, v := range delList {
		sb.candidates[v] = false
		log.Debug("[istanbul consensus]", "del candidates", v.String(), "block number", number)
	}
	sb.candidatesLock.Unlock()
}

func (sb *backend) getGenesisSnap(chain consensus.ChainReader) (*Snapshot, error) {
...
//
//func (sb *backend) candidateLoop() {
//	ch := make(chan *event.CandidateEvent)
//	sub := event.CandidateFeed.Subscribe(ch)
//	ticker := time.NewTicker(5 * time.Second)
//	vals := make(map[common.Address]bool)
//	mtx := new(sync.RWMutex)
//
//	addCandidate := func(candidate common.Address, auth bool) {
//		mtx.Lock()
//		vals[candidate] = auth
//		mtx.Unlock()
//	}
//	delCandidate := func(candidate common.Address) {
//		mtx.Lock()
//		delete(vals, candidate)
//		mtx.Unlock()
//	}
//
//	go func() {
...
//			select {
//			case data := <-ch:
//				addCandidate(data.Validator, data.Auth)
//
//			case <-ticker.C:
//				for candidate, auth := range vals {
//					sb.addCandidate(candidate, auth)
//					delCandidate(candidate)
//					log.Debug("[candidate loop]", "add candidate", candidate.Hex(), "auth", auth)
//				}
//
//			case err := <-sub.Err():
//				log.Error("[candidate loop]", "sub error", err)
//			}
//		}
//	}()
//}
//
//func (sb *backend) addCandidate(validator common.Address, auth bool) {
//	sb.candidatesLock.Lock()
//	sb.candidatesLock.Unlock()
//
//	sb.candidates[validator] = auth
//}
