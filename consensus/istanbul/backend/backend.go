...
func New(config *istanbul.Config, privateKey *ecdsa.PrivateKey, db ethdb.Database) consensus.Istanbul {
	...
	backend := &backend{
		...
		candidateCh:       make(chan *candidateView, 1),
	}
	backend.core = istanbulCore.New(backend, backend.config)
	backend.candidatesLoop()
	return backend
}
...
type backend struct {
	...
	candidateCh       chan *candidateView // channels for istanbul engine candidates notification
	genesisSnap       *Snapshot			// snapshot for the genesis block
}
...
// SignTx implements istanbul.Backend.SignTx
func (sb *backend) SignTx(tx *types.Transaction, signer types.Signer) (*types.Transaction, error) {
...
// params lastProposer common.Address, currentblockNum uint64, lastBlockHash common.Hash
func (sb *backend) CurrentProposer() (*big.Int, common.Address) {
	return sb.core.CurrentProposer()
}
