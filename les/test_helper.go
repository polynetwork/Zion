...
func prepare(n int, backend *backends.SimulatedBackend) {
	...
	for i := 0; i < n; i++ {
		...
		case 0:
			...
			tx, _ := types.SignTx(types.NewTransaction(nonce, userAddr1, big.NewInt(100000000000000000), params.TxGas, big.NewInt(params.InitialBaseFee), nil), signer, bankKey)
		...
		case 1:
			...
			tx1, _ := types.SignTx(types.NewTransaction(bankNonce, userAddr1, big.NewInt(1000), params.TxGas, big.NewInt(params.InitialBaseFee), nil), signer, bankKey)
			backend.SendTransaction(ctx, tx1)

			// user1 relays ether to user2
			tx2, _ := types.SignTx(types.NewTransaction(userNonce1, userAddr2, big.NewInt(1000), params.TxGas, big.NewInt(params.InitialBaseFee), nil), signer, userKey1)
